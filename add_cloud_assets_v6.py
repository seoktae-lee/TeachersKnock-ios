...
def fill_holes(mask_img):
    ...
    for sx, sy in seeds:
         if pixels[sx, sy] == 0: # If it's black (background)
             queue.append((sx, sy))
             bg_pixels[sx, sy] = 255 # Valid External Background
        ...
        for dx, dy in shifts:
            ...
            if 0 <= nx < width and 0 <= ny < height:
                if bg_pixels[nx, ny] == 0: # Not visited
                    if pixels[nx, ny] == 0: # Background in original
                        bg_pixels[nx, ny] = 255
                        queue.append((nx, ny))
                        
    # Anything not reached (bg_pixels==0) is Object
...
def process_image(img, level):
    ...
    # Safe Zone (For Lv 2, 3 where head is white)
    # Also good for Lv 6 if it has a head
    has_safe_zone = level in [2, 3] # Only rigidly apply for 2/3 where head=bg color
    safe_zone_center = (width // 2, int(height * 0.35))
    safe_zone_radius_sq = (width * 0.28) ** 2 # Increased radius slightly
    
    # Check if Dark Background (Lv 1, 4, 5, 6)
    # We use this to tune Thresholds
    corners = [pixels[0,0], pixels[width-1,0], pixels[0, height-1], pixels[width-1, height-1]]
    avg_bg = sum(sum(c) for c in corners) / 12.0
    is_dark_bg = avg_bg < 150
    
    print(f"Level {level}: DarkBG={is_dark_bg}, SafeZone={has_safe_zone}")
    ...
    for y in range(height):
        for x in range(width):
            ...
            if 140 <= h <= 270:
                 if is_dark_bg:
                     # On Dark BG, anything saturated is cloud
                     if s > 0.05: is_object = True
                 else:
                     # On Light BG, need higher saturation to distinguish from gray shadows
                     # Cloud is S~0.15, BG S~0.02.
                     if s > 0.08: is_object = True
            
            # 2. Dark Outline (Only for Dark BG/Dark Outlines)
            if v < 0.25: is_object = True
            
            # 3. Spatial Lock (Force Keep Head)
            if has_safe_zone:
