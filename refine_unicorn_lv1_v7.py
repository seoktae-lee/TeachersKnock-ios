...
def process_lv1_new_source(img):
    img = img.convert("RGB")
    ...
    # "Nuclear" Thresholding (Strict)
    # Background is gray (S ~ 0.05). Egg is Gold (S > 0.2).
    # New image is said to be "distinct", so we can trust the saturation more.
    ...
    for y in range(height):
        for x in range(width):
            ...
            if s > 0.20: is_egg = True # Slightly relaxed from 0.25 to catch contour
            
            # 2. Value Check (Sparkles)
            if v > 0.95: is_egg = True
            
            # 3. Brightness Floor
            if v < 0.25: is_egg = False 
    ...
    # Erosion
    # User said "distinct contour", so we assume the edge is good.
    # But "background removal along contour" means we should shave it slightly to be safe.
    # 3px is gentle.
    final_mask = final_mask.filter(ImageFilter.MinFilter(3))
    
    return final_mask

def resize_and_canvas(img, mask, target_height=700, canvas_size=(1024, 1024)):
    width, height = img.size
...
print("Refining Lv 1 (V7: New Source)...")
path = os.path.join(artifacts_dir, filename)

if os.path.exists(path):
    img = Image.open(path)
    mask = process_lv1_new_source(img)
    
    img = resize_and_canvas(img, mask, target_height=700)
