...
def smooth_mask(mask, upscale_factor=4, blur_radius=15):
    # Vector Smoothing Technique
    w, h = mask.size
    new_w, new_h = w * upscale_factor, h * upscale_factor
    
    # Upscale (Bilinear for soft edges)
    ...
    # Downscale (Lanczos for AA)
    small_mask = big_mask.resize((w, h), Image.Resampling.LANCZOS).convert('L')
    
    # Sharp Binary again? Or keep AA? 
    # User wants "Smooth" but not "Blurry".
    # Let's clean it to binary for the core shape, then the resize later provides AA.
    small_mask = small_mask.point(lambda x: 255 if x > 127 else 0)
    
    return small_mask

def process_lv1_v10(img):
    img = img.convert("RGB")
    ...
    # 1. Strict Background Selection
    for y in range(height):
        for x in range(width):
            r, g, b = pixels[x, y]
            h, s, v = rgb_to_hsv(r, g, b)
            is_egg = False
            if s > 0.25: is_egg = True # Strict Gold
            if v > 0.95: is_egg = True # Strict Sparkle
            if v < 0.30: is_egg = False # Kill shadows
    ...
    # 4. Erosion (Pre-Smoothing)
    final_mask = final_mask.filter(ImageFilter.MinFilter(5))
    
    # 5. SMOOTHING (The key fix for "jagged contour")
    final_mask = smooth_mask(final_mask)
    
    # 6. ALPHA EROSION (The key fix for "White Border")
    # Determine the edge and push it in.
    # MinFilter(3) on the mask is basically Alpha Erosion since the mask IS the alpha.
    final_mask = final_mask.filter(ImageFilter.MinFilter(3))
...
print("Refining Lv 1 (V10: Smooth, Alpha Eroded, 710px)...")
path = os.path.join(artifacts_dir, filename)

if os.path.exists(path):
    img = Image.open(path)
    mask = process_lv1_v10(img)
