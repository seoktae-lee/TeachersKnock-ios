...
def process_lv1_distinct_outline(img):
    img = img.convert("RGB")
    ...
    # 1. Nuclear Thresholding (Strict Background Removal)
    ...
    # 3. Erosion (To make the egg smaller/sharper before outlining)
    final_mask = final_mask.filter(ImageFilter.MinFilter(9))
    
    # --- NEW: Generate DISTINCT GOLD Outline ---
    # Dilate by 5px (Kernel size 5 -> Radius ~2px extension)
    dilated_mask = final_mask.filter(ImageFilter.MaxFilter(5))
    
    # Subtract inner mask to get the ring
...
def resize_and_canvas_with_outline(img, mask, outline_mask, target_height=700, canvas_size=(1024, 1024)):
    ...
    for i in range(len(data)):
        if mask_data[i] > 0:
            ...
            # On Outline: Use SOLID Gold color with alpha scaling from the mask if needed
            # But MaxFilter produces a binary-ish mask (0 or 255 mostly)
            # Let's force solid gold if outline_data > 0
...
print("Refining Lv 1 (V5: Distinct Gold Line)...")
path = os.path.join(artifacts_dir, filename)

if os.path.exists(path):
    img = Image.open(path)
    mask, outline_mask = process_lv1_distinct_outline(img)
