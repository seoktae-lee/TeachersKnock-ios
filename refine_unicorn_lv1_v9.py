...
def process_lv1_final_v9(img):
    ...
    # Erosion: 7px (Same as V8 fixed)
    final_mask = final_mask.filter(ImageFilter.MinFilter(7))
    
    return final_mask

def resize_and_canvas_padded(img, mask, target_height=670, canvas_size=(1024, 1024)):
    # Create RGBA
    ...
    # Resize
    # Target Height 670 (Matching Cloud's 671)
...
print("Refining Lv 1 (V9: Target Height 670)...")
path = os.path.join(artifacts_dir, filename)

if os.path.exists(path):
    img = Image.open(path)
    mask = process_lv1_final_v9(img)
    
    img = resize_and_canvas_padded(img, mask, target_height=670)
